...
HHH_TT_X_1or2tag(bool scaled=true, bool log=true, float min=0.1, float max=-1., string inputfile="root/$HISTFILE", const char* directory="tauTau_$CATEGORY")
{
  ...
  if(std::string(directory) == std::string("tauTau_2jet1tag"  )){ dataset= "#tau_{h}#tau_{h}, 2jet-1tag";     }
  ...
  if(std::string(directory) == std::string("tauTau_2jet2tag"  )){ dataset = "#tau_{h}#tau_{h}, 2jet-2tag";              }
  ...
  //    dataset = category+" ,"+category_extra;
    }
  }
#else
